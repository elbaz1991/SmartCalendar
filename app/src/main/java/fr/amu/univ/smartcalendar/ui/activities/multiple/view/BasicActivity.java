    ...
    public List<? extends WeekViewEvent> onMonthChange(int newYear, int newMonth) {
        ...
        if(eventsList!=null) {
            for (Evenement e : eventsList) {
                ...
                WeekViewEvent event;
                if(DateFormater.getDifferenceDays(e.getDateDebut(),e.getDateFin()) > 0)
        ...
        /*
        if(eventsDates != null) {
            for (Long dateEvent : eventsDates) {
                List<Evenement> eventsList = evenementDAO.findByDateEvent(new Date(dateEvent));
                if(eventsList!=null) {
                    for (Evenement e : eventsList) {
                        Calendar startTime = Calendar.getInstance();
                        Calendar endTime = Calendar.getInstance();
                        startTime.setTimeInMillis(e.getDateDebut());
                        endTime.setTimeInMillis(e.getDateFin());
                        String titre = eventViewAdapter.formatTitre(e.getTitre(),e.getDateDebut(),e.getDateFin(),dateEvent);
                        WeekViewEvent event = new WeekViewEvent(e.getId(), titre, null,startTime, endTime,true);
                        event.setColor(e.getColor());
                        events.add(event);
                    }
                }
